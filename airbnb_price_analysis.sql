-- Creating table
CREATE TABLE bookings (
	booking_id INT PRIMARY KEY,
	listing_name VARCHAR,
	host_id INT,
	host_name VARCHAR(50),
	neighbourhood_group VARCHAR(30),
	neighbourhood VARCHAR(30),	
	latitude DECIMAL(11,8),
	longitude DECIMAL(11,8),
	room_type VARCHAR(30),
	price INT,
	minimum_nights INT,
	num_of_reviews INT,
	last_review DATE,
	reviews_per_month DECIMAL(4,2),
	calculated_host_listings_count INT,
	availability_365 INT
);

-- Checking all data
SELECT *
FROM bookings;

-- Choosing what columns to use and saving the new table as bookings_clean
CREATE TABLE bookings_clean AS
SELECT
    booking_id,
    listing_name,
    host_name,
    neighbourhood_group,
    neighbourhood,
    price,
    last_review
FROM bookings;

-- Checking the data again
SELECT *
FROM bookings_clean;

-- Calculating Average, Max, and Min price
SELECT
	booking_id,
	listing_name,
	neighbourhood_group,
	ROUND(AVG(price) OVER(), 2) AS avg_price,
	MIN(price) OVER() AS min_price,
	MAX(price) OVER() AS max_price
FROM bookings;

-- Calculating the difference from average price
SELECT
	booking_id,
	listing_name,
	neighbourhood_group,
	price,
	ROUND(AVG(price) OVER(), 2) AS avg_price,
	ROUND((price - AVG(price) OVER()), 2) AS diff_from_avg
FROM bookings;

-- Calculating percent of average price
SELECT
	booking_id,
	listing_name,
	neighbourhood_group,
	price,
	ROUND(AVG(price) OVER(), 2) AS avg_price,
	ROUND((price / AVG(price) OVER() * 100), 2) AS percent_of_avg_price
FROM bookings;

-- Calculating percent difference from average price
SELECT
	booking_id,
	listing_name,
	neighbourhood_group,
	price,
	ROUND(AVG(price) OVER(), 2) AS avg_price,
	ROUND((price / AVG(price) OVER() - 1) * 100, 2) AS percent_diff_from_avg_price
FROM bookings;

-- Calculating average price partitioned by neighbourhood groups
SELECT
	booking_id,
	listing_name,
	neighbourhood_group,
	neighbourhood,
	price,
	ROUND(AVG(price) OVER(PARTITION BY neighbourhood_group), 2) AS avg_price_by_neigh_group
FROM bookings;

-- Calculating average price partitioned by neighbourhood groups and neighbourhoods
SELECT
	booking_id,
	listing_name,
	neighbourhood_group,
	neighbourhood,
	price,
	ROUND(AVG(price) OVER(PARTITION BY neighbourhood_group), 2) AS avg_price_by_neigh_group,
	ROUND(AVG(price) OVER(PARTITION BY neighbourhood_group, neighbourhood), 2) AS avg_price_by_group_and_neigh
FROM bookings;

-- Calculating the difference from average price by neighborhood group and neighborhood group delta
SELECT
	booking_id,
	listing_name,
	neighbourhood_group,
	neighbourhood,
	price,
	ROUND(AVG(price) OVER(PARTITION BY neighbourhood_group), 2) AS avg_price_by_neigh_group,
	ROUND(AVG(price) OVER(PARTITION BY neighbourhood_group, neighbourhood), 2) AS avg_price_by_group_and_neigh,
	ROUND(price - AVG(price) OVER(PARTITION BY neighbourhood_group), 2) AS neigh_group_delta,
	ROUND(price - AVG(price) OVER(PARTITION BY neighbourhood_group, neighbourhood), 2) AS group_and_neigh_delta
FROM bookings;

-- Ranking overall price using ROW_NUMBER
SELECT
	booking_id,
	listing_name,
	neighbourhood_group,
	neighbourhood,
	price,
	ROW_NUMBER() OVER(ORDER BY price DESC) AS overall_price_rank
FROM bookings;

-- Ranking price for different neighbourhood groups
SELECT
	booking_id,
	listing_name,
	neighbourhood_group,
	neighbourhood,
	price,
	ROW_NUMBER() OVER(ORDER BY price DESC) AS overall_price_rank,
	ROW_NUMBER() OVER(PARTITION BY neighbourhood_group ORDER BY price DESC) AS neigh_group_price_rank
FROM bookings;

-- Finding the top 3 neighbourhoods ranked by price
SELECT
	booking_id,
	listing_name,
	neighbourhood_group,
	neighbourhood,
	price,
	ROW_NUMBER() OVER(ORDER BY price DESC) AS overall_price_rank,
	ROW_NUMBER() OVER(PARTITION BY neighbourhood_group ORDER BY price DESC) AS neigh_group_price_rank,
	CASE
		WHEN ROW_NUMBER() OVER(PARTITION BY neighbourhood_group ORDER BY price DESC) <= 3 THEN 'Yes'
		ELSE 'No'
	END AS top3_flag
FROM bookings;

-- Filtering the results to only include rankings 3 and below
SELECT * FROM (
	SELECT
		booking_id,
		listing_name,
		neighbourhood_group,
		neighbourhood,
		price,
		ROW_NUMBER() OVER(ORDER BY price DESC) AS overall_price_rank,
		ROW_NUMBER() OVER(PARTITION BY neighbourhood_group ORDER BY price DESC) AS neigh_group_price_rank,
		CASE
			WHEN ROW_NUMBER() OVER(PARTITION BY neighbourhood_group ORDER BY price DESC) <= 3 THEN 'Yes'
			ELSE 'No'
		END AS top3_flag
	FROM bookings
	) a
WHERE top3_flag = 'Yes'

-- Comparing ranks with ROW_NUMBER with RANK functions between overall price and neighbourhood groups
SELECT
	booking_id,
	listing_name,
	neighbourhood_group,
	neighbourhood,
	price,
	ROW_NUMBER() OVER(ORDER BY price DESC) AS overall_price_rank,
	ROW_NUMBER() OVER(PARTITION BY neighbourhood_group ORDER BY price DESC) AS neigh_group_price_rank,
	RANK() OVER(ORDER BY price DESC) AS overall_price_rank_with_rank,
	RANK() OVER(PARTITION BY neighbourhood_group ORDER BY price DESC) AS neigh_group_price_rank_with_rank
FROM bookings;

-- Comparing rank of price with ROW_NUMBER, RANK, and DENSE_RANK
SELECT
	booking_id,
	listing_name,
	neighbourhood_group,
	neighbourhood,
	price,
	ROW_NUMBER() OVER(ORDER BY price DESC) AS overall_price_rank,
	RANK() OVER(ORDER BY price DESC) AS overall_price_rank_with_rank,
	DENSE_RANK() OVER(ORDER BY price DESC) AS overall_price_rank_with_dense_rank
FROM bookings;

-- Calculating LAG of price by one record
SELECT
	booking_id,
	listing_name,
	host_name,
	price,
	last_review,
	LAG(price) OVER(PARTITION BY host_name ORDER BY last_review)
FROM bookings;














